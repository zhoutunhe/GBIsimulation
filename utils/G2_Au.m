...
function [T10,Transmission,Phase,objects]=G2_Au(thick_gra,thick_sub,p1,w1,N,dx0,energies,X,x0,y0,plotTransmission)
energymiddle = 25e3; %25keV
% Ni grating
index=2;
...
if isempty(n.deltaSpectrum) || isempty(n.betaSpectrum)
    ...
    elseif strcmpi(n.type,'Composition')
        ...
        material= commonMaterials(25);   %the index of common materials 25 is gold
...
if ~thick_gra
    ...
    objects(index).thickness=thick_gra; %45e-6;   %[m], from fitting result
end
k = energymiddle*qe/(c*hbar);
% phaseshift = objects(2).thickness*n.deltaSpectrum*k;
...
obj.width=inf; %xmax-xmin;
obj.height=inf; %ymax-ymin;
obj.thickness=thick_sub; %500e-6;   %[m]
obj.period=0;
obj.name='Substrate';
obj.rotation=0; %[rad]
obj.x=0;
...
if isempty(n.deltaSpectrum) || isempty(n.betaSpectrum)
    ...
    elseif strcmpi(n.type,'Composition')
        commonMaterials = getCommonMaterials();
        material= commonMaterials(30);   %the index of common materials 30 is silicon
        n.density=material.density;
        n.formulas=material.formulas;
        n.relativeWeights=material.relativeWeights;
...
for j=1:N
    for l=1:round(N*dx0/p1); %number of periods of the grating
        ...
        if (j*dx0-l*p1)/w1<=0&&(j*dx0-l*p1)/w1>-0.999999
